...
describe("shuffleArray and unshuffleArray", () => {
  it("should shuffle and then correctly unshuffle an array using params1277", () => {
    ...
    const originalArr: number[] = Array.from(
      { length: params1277.P },
      (_, i) => i,
    );
  ...
  it("should throw an error if array length does not match params.P using params1277", () => {
    ...
    const expectedErrorMessage = (len: number) =>
      ErrorType.SliceLengthNotR3Size;

    expect(() => shuffleArray(shortArr, seed, params1277)).toThrow(
      expectedErrorMessage(shortArr.length),
    );
    expect(() => shuffleArray(longArr, seed, params1277)).toThrow(
      expectedErrorMessage(longArr.length),
    );

    expect(() => unshuffleArray(shortArr, seed, params1277)).toThrow(
      expectedErrorMessage(shortArr.length),
    );
    expect(() => unshuffleArray(longArr, seed, params1277)).toThrow(
      expectedErrorMessage(longArr.length),
    );
  });
