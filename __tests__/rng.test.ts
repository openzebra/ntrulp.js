...
describe('random functions', () => {
  ...
  test('randomSmall should return array with length P containing values -1, 0, 1', () => {
    const rng = createSeededRng();
    
    for (let i = 0; i < 100; i++) {
      const result = randomSmall(rng);
  ...
  test('shortRandom should return valid array with sum of absolute values equal to W', () => {
    const rng = createSeededRng();
    
    for (let i = 0; i < 100; i++) {
      const result = shortRandom(rng);
